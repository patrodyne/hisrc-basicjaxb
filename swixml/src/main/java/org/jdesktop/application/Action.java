 ...
 * target or "actions" class. Typically applications will use
 * {@link ApplicationContext#getActionMap(Class, Object) getActionMap} to lazily
 * construct and cache ApplicationActionMaps, rather than constructing them
 * directly. By default the ApplicationActionMap's
 * {@link ApplicationActionMap#get key} for an &#064;Action is the name of the
 * method. The <code>name</code> parameter can be used to specify a different
 * key.
 * 
 * <p>
 * The <code>ApplicationAction's</code> properties are initialized with
 * resources loaded from a ResourceBundle with the same name as the actions
 * class. The list of properties initialized this way is documented by the
 * {@link ApplicationAction ApplicationAction's} constructor.
 * 
 * <p>
 * The method marked with &#064;Action, can have no parameters, or a single
 * ActionEvent parameter. The method's return type can be <code>void</code> or
 * {@link Task}. If the return type is Task, the Task will be executed by the
 * ApplicationAction's <code>actionPerformed</code> method.
 * 
 * <p>
