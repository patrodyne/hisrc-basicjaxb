 ...
 * to monitor the three key SwingWorker methods: {@code
 * doInBackground}, {@code process} and {@code done}.
 * 
 * <p>
 * When a Task completes, the {@code final done} method invokes one of
 * {@code succeeded}, {@code cancelled}, {@code interrupted}, or {@code failed}.
 * The {@code final done} method invokes {@code finished} when the completion
 ...
 * {@link #message message}, which is used to set the {@code message} property.
 * </p>
 * 
 * <p>
 ...
 * </code></pre>
 * 
 * <p>Typically the resources for this class would be defined in the MyTask
 ...
 * {@code Task.InputBlocker}. For example, assume that {@code busyGlassPane} is
 * a component that consumes (and ignores) keyboard and mouse input:
 * </p>
 * 
 * <pre>
...
public abstract class Task<T, V> extends SwingWorker<T, V>
{
	 ...
	 * <p>
	 * Construct a {@code Task}. If the {@code resourceMap} parameter is not
	 * null, then the {@code title}, {@code description}, and {@code message}
	 * properties are initialized from resources. The {@code resourceMap} is
	 * also used to lookup localized messages defined with the {@link #message
	 * message} method. In both cases, if the value of {@code resourcePrefix} is
	 * not null or an empty string {@code ""}, resource names must have the name
	 * of the {@code resourcePrefix} parameter, followed by a ".", as a prefix
	 * 
	 ...
	 * supported with the InputBlocker's resourceMap property.
	 * <p>
	 * Construct a {@code Task} with the specified resource name prefix, whose
	 ...
	 * etc..) using the same naming convention.
	 * 
	...
	/**
	 * Set the {@code title} property. The default value of this property is the
	 * value of the {@link #getResourceMap resourceMap's} {@code title}
	 * resource.
	 * <p>
	 ...
	 * of this property is the value of the {@link #getResourceMap
	 * resourceMap's} {@code description} resource.
	 * <p>
	 ...
	 * formatResourceKey} names a resource whose value is a format string. See
	 ...
	 * {@code message} property to {@code formatResourceKey}.
	 * 
	 ...
	 * 
	 * <p>This method runs on the EDT. It does nothing by default.</p>
	 *
	 ...
	 * when the Task's state changes to {@code STARTED}, each time the
	 * {@code process} method is called, and when the Task's state changes to
	 * {@code DONE}. All of the listener methods will run on the event
	 * dispatching thread.
	 * 
	 ...
	 * {@code block} and {@code unblock} that do the work of blocking the GUI.
	 * For the sake of input blocking, a Task begins executing when it's
	 * {@link TaskService#execute submitted} to a {@code TaskService}, and it
	 * finishes executing after the Task's completion methods have been called.
	 * 
	 * <p>The InputBlocker's {@link Task.BlockingScope BlockingScope} and the
	 * blocking {@code target} object define what part of the GUI's input will
	 * be blocked:</p>
	 * 
	 * <dl>
	 *   <dt><b><code>Task.BlockingScope.NONE</code></b></dt>
	 *   <dd>Don't block input. The blocking target is ignored in this case.</dd>
	 *   <dt><b><code>Task.BlockingScope.ACTION</code></b></dt>
	 *   <dd>Disable the target {@link javax.swing.Action Action} while the Task is executing.</dd>
	 *   <dt><b><code>Task.BlockingScope.COMPONENT</code></b></dt>
	 *   <dd>Disable the target {@link java.awt.Component} Component while the Task is executing.</dd>
	 *   <dt><b><code>Task.BlockingScope.WINDOW</code></b></dt>
	 *   <dd>Block the Window ancestor of the target Component while the Task is executing.</dd>
	 *   <dt><b><code>Task.BlockingScope.Application</code></b></dt>
	 *   <dd>Block the entire Application while the Task is executing. The blocking target is ignored in this case.</dd>
	 * </dl>
	 * 
	 * <p>Input blocking begins when the {@code block} method is called and ends
	 * when {@code unblock} is called. Each method is only called once,
	 * typically by the {@code TaskService}.</p>
	 * 
		...
		AbstractBean
	{
		 ...
		 * then an exception is thrown. If scope is {@code BlockingScope.ACTION}
		 * then target must be a {@link javax.swing.Action Action}. If scope is
		 * {@code BlockingScope.WINDOW} or {@code BlockingScope.COMPONENT} then
