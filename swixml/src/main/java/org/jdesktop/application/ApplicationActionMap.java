 ...
 * to an <code>&#064;Action</code> method from a single <code>actionsClass</code> (i.e.
 * a class that contains one or more <code>&#064;Actions</code>). Each entry's key
 * is the <code>&#064;Action's</code> name (the method name by default), and the
 * value is an {@link ApplicationAction} that calls the <code>&#064;Actions</code>
 * method. For example, the code below prints <code>"Hello World"</code>:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * <p>If a <code>ResourceMap</code> is provided then each <code>ApplicationAction's</code>
 * ({@link javax.swing.Action#putValue putValue},
 * {@link javax.swing.Action#getValue getValue}) properties are initialized from
 * the ResourceMap.</p>
 * 
 * <p>TBD: explain use of resourcemap including action types, actionsObject,
	...
	private void addAnnotationActions(ResourceMap resourceMap)
	{
		...
		for ( Method m : actionsClass.getDeclaredMethods() )
		{
			...
			if ( action != null )
			{
				...
				ApplicationAction appAction = new ApplicationAction(this, resourceMap, actionName, m, enabledProperty,
