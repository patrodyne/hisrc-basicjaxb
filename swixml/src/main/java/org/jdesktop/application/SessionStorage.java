 ...
 * {@code SessionState.Property} object has been defined. SessionState
 * Properties that preserve the {@code bounds} {@code Rectangle} for Windows,
 * the {@code dividerLocation} for {@code JSliderPanes} and the
 * {@code selectedIndex} for {@code JTabbedPanes} are defined by default. The
 * {@code ApplicationContext} {@link ApplicationContext#getSessionStorage
 * getSesssionStorage} method provides a shared {@code SessionStorage} object.
 * </p>
 * 
 * <p>
 * A typical Application saves session state in its {@link Application#shutdown
 * shutdown()} method, and then restores session state in
 * {@link Application#startup startup()}:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * <p>In this example, the bounds of {@code mainFrame} as well the session state
 * for any of its {@code JSliderPane} or {@code
 * JTabbedPane} will be saved when the application shuts down, and restored when
 ...
 * the {@code LocalStorage} {@code LocalStorage#save save} and
 * {@code LocalStorage#save load} methods. The {@code startup} method must set
 ...
 * pathname for filename {@code "session.xml"} is typically:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * <p>Where the value of {@code ${userHome}} is the the value of the Java System
...
public class SessionStorage
{
	...
	/**
	 * <p>Constructs a SessionStorage object. The following {@link Property
	 * Property} objects are registered by default:</p>
	 * 
	 * <table border="1">
	 * <caption><b>Property Objects</b></caption>
	 * <tr>
	 ...
	 * </table>
	 * 
	 * <p>Applications typically would not create a {@code SessionStorage} object
	 ...
	 * Component c doesn't have a name per getComponentName(). The pathname is
	 * basically the name of all of the components, starting with c, separated
	 * by "/". This path is the reverse of what's typical, the first path
	 * element is c's name, rather than the name of c's root Window or Applet.
	 * That way pathnames can be distinguished without comparing much of the
	 * string. The names of intermediate components *can* be null, we substitute
	 * "[type][z-order]" for the name. Here's an example:
	 * 
	 * <pre> JFrame myFrame = new JFrame(); // anonymous JPanel subclass JPanel
	 * p = new JPanel() {}; JButton myButton = new JButton();
	 * myButton.setName("myButton"); p.add(myButton); myFrame.add(p);
	 * 
	 ...
	 * names to it's children (layeredPane, contentPane, glassPane); all AWT
	 * components lazily compute a name. If we hadn't assigned the JFrame a
	 * name, it's name would have been "frame0".
	 */
		...
		while ((c.getParent() != null) && !(c instanceof Window))
		{
			...
			if ( name == null )
			{
				...
				else
				{
					...
					logger.warn("Couldn't compute pathname for " + c);
					return null;
				}
			}
	...
	/*
	 * Recursively walk the component tree, breadth first, storing the state -
	 * Property.getSessionState() - of named components under their pathname
	 * (the key) in stateMap.
	 * 
	 * Note: the breadth first tree-walking code here should remain structurally
	...
	/**
	 * <p>Saves the state of each named component in the specified hierarchy.
	 ...
	 * {@link Property#getSessionState state} is saved.</p>
	 * 
	 * <p>Component names can be any string however they must be unique relative to
	 ...
	 * 
	 * <p>The type of sessionState values (i.e. the type of values returned by
	 * {@code Property.getSessionState}) must be one those supported by
			...
			if ( root != null )
			{
				...
				if ( p != null )
				{
					...
					if ( pathname != null )
					{
						...
						{
							logger.warn("No saved state for " + root);
						}
					}
				}
			}
	 ...
	 * state loaded from a file using {@link LocalStorage}. Each component is visited
	 * in breadth-first order: if a {@link #getProperty(Component) Property} exists
	 * for that component, and the component has a {@link java.awt.Component#getName
	 * name}, then its state is {@link Property#setSessionState restored}.
	 * 
	...
	public interface Property
	{
		 ...
		 * Java bean or a Collection the defines the {@code Component} state
		 ...
		 * {@code setSessionState} to restore the Component's state.
		 * 
	 ...
	 * {@code GraphicsConfiguration}, i.e. the bounds of the screen that the
	 ...
	 * <p>
	 * This class defines how the session state for {@code Windows} is
	 * {@link WindowProperty#getSessionState saved} and and
	 * {@link WindowProperty#setSessionState restored} in terms of a property
	 * called {@code sessionState}. The Window's {@code bounds Rectangle} is
	 ...
	 * {@code JWindow}, {@code JDialog}, and {@code JFrame}.
	 * 
	 * @see #save
	 * @see #restore
	 * @see WindowState
		...
		Property
	{
		 ...
		 * ({@code GraphicsConfiguration}) haven't changed, the number of
		 * screens hasn't changed, and the {@link Window#isLocationByPlatform
		 ...
		 * {@code GraphicsConfiguration} {@link GraphicsConfiguration#getBounds
		 * bounds} matches the {@link WindowState#getGraphicsConfigurationBounds
		 * WindowState's value}, and Window {@code c's}
		 * {@link Window#isLocationByPlatform isLocationByPlatform} property is
		 * false, then set the Window's to the {@link WindowState#getBounds
		 * saved value}.
		 * <p>
	 ...
	 * TabbedPaneState} object created by
	 * {@link TabbedPaneProperty#getSessionState} and used to restore the
	 ...
	 * <p>
	 * This class defines how the session state for {@code JTabbedPanes} is
	 * {@link WindowProperty#getSessionState saved} and and
	 * {@link WindowProperty#setSessionState restored} in terms of a property
	 * called {@code sessionState}. The JTabbedPane's {@code selectedIndex} is
	 * saved and restored if the number of tabs ({@code tabCount}) hasn't
	 * changed.
	 * <p>
	 * {@code TabbedPaneProperty} is registered for {@code
	 * JTabbedPane.class} by default, so this class applies to JTabbedPane and
		...
		Property
	{
		 ...
		 * <p>
		 * Throws an {@code IllegalArgumentException} if {@code c} is not a
		 * {@code JTabbedPane} or if {@code state} is non-null but not an
	 ...
	 * <p>
	 * This class defines how the session state for {@code JSplitPanes} is
	 * {@link WindowProperty#getSessionState saved} and and
	 * {@link WindowProperty#setSessionState restored} in terms of a property
	 * called {@code sessionState}. The JSplitPane's {@code dividerLocation} is
	 * saved and restored if its {@code orientation} hasn't changed.
	 * <p>
	 * {@code SplitPaneProperty} is registered for {@code
	 * JSplitPane.class} by default, so this class applies to JSplitPane and any
		...
		Property
	{
		 ...
		 * {@code JSplitPane c}. If the split pane's {@code dividerLocation} is
		 ...
		 * 
		 * @param c the {@code JSplitPane} whose dividerLocation will recoreded
		 *            in a {@code SplitPaneState} object.
	...
	/**
	 * This Java Bean records the {@code columnWidths} for all of the columns in
	 * a JTable. A width of -1 is used to mark {@code TableColumns} that are not
	 * resizable.
	 * 
	 * @see TableProperty
	 * @see #save
	 ...
	 * <p>
	 * This class defines how the session state for {@code JTables} is
	 * {@link WindowProperty#getSessionState saved} and and
	 * {@link WindowProperty#setSessionState restored} in terms of a property
	 * called {@code sessionState}. We save and restore the width of each
	 * resizable {@code TableColumn}, if the number of columns haven't changed.
	 * <p>
	 * {@code TableProperty} is registered for {@code
	 * JTable.class} by default, so this class applies to JTable and any
		...
		Property
	{
		...
		/**
		 * Returns a {@link TableState TableState} object for {@code JTable c}
		 ...
		 * 
		 * @param c the {@code JTable} whose columnWidths will be saved in a
		 *            {@code TableState} object.
		...
		/**
		 * Restore the width of each resizable {@code TableColumn}, if the
		 * number of columns haven't changed.
		 * <p>
		 * Throws an {@code IllegalArgumentException} if {@code c} is not a
		 * {@code JTable} or if {@code state} is not an instance of
		 * {@link TableState}.
		 * 
	...
	/**
	 * Returns the {@code Property} object that was {@link #putProperty
	 ...
	 * {@code Property} for a {@code Component} use
	 ...
	 * @return the {@code Property} registered with {@code putProperty} for the
	...
	/**
	 * Register a {@code Property} for the specified class. One can clear the
	 * {@code Property} for a class by setting the entry to null:
	 * 
	 * <pre>
	...
	/**
	 * If a {@code sessionState Property} object exists for the specified
	 ...
	 * {@code sessionState Property} object for each component to whose session
	 * state is to be saved or restored.
	 * <p>
	 * The {@code putProperty} method registers a Property object for a class.
	 ...
	 *         {@link javax.swing.JComponent#getClientProperty client property}
	 *         under (client property key) {@code SessionState.Property.class},
	 *         then return that, otherwise return the value of
