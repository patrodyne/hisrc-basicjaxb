 ...
 * </pre>
 * 
 * <p>{@link ApplicationContext#getResourceMap(Class)
 ...
 * 
 * <p>Resources for a class are defined by an eponymous {@code ResourceBundle} in a
 * {@code resources} sub-package. The Application class itself may also provide
 ...
 * 
 * <p>The mapping from classes and {@code Application} to a list ResourceBundle
 * names is handled by two protected methods: {@link #getClassBundleNames(Class)
 ...
 * additional ResourceBundle names to the default lists.</p>
 * 
...
public class ResourceManager extends AbstractBean
{
	...
	private ResourceMap getApplicationResourceMap()
	{
		if ( appResourceMap == null )
		{
			...
			if ( appClass == null )
			{
				logger.warn("getApplicationResourceMap(): no Application class");
	 ...
	 * that encapsulate the {@code ResourceBundles} for each class from
	 * {@code startClass} to (including) {@code stopClass}. The final link in
	 * the chain is Application ResourceMap chain, i.e. the value of
	 * {@link #getResourceMap() getResourceMap()}.
	 * 
	 * <p>The ResourceBundle names for the chain of ResourceMaps are defined by
	 * {@link #getClassBundleNames} and {@link #getApplicationBundleNames}.
	 * Collectively they define the standard location for
	 * {@code ResourceBundles} for a particular class as the {@code resources}
	 * subpackage. For example, the ResourceBundle for the single class
	 * {@code com.myco.MyScreen}, would be named
	 * {@code com.myco.resources.MyScreen}. Typical ResourceBundles are
	 * ".properties" files, so: {@code
	 * com/foo/bar/resources/MyScreen.properties}. The following table is a list
	 * of the ResourceMaps and their constituent ResourceBundles for the same
	 * example:</p>
	 * 
	 * <table border="1">
	 ...
	 * </table>
	 * 
	 * <p>None of the ResourceBundles are required to exist. If more than one
	 * ResourceBundle contains a resource with the same name then the one
	 * earlier in the list has precedence</p>
	 * 
	 * <p>ResourceMaps are constructed lazily and cached. One ResourceMap is
	 ...
	 *         {@code ResourceBundles} found in the resources sub-package for
	 *         each class.
	 *         
	 ...
	 * convenience method, it's the same as:
	 ...
	 * 
	 * <p>The default value for this property is a list of
	 * {@link #getClassBundleNames per-class} ResourceBundle names, beginning
	 * with the {@code Application's} class and of each of its superclasses, up
	 * to {@code Application.class}. For example, if the Application's class was
	 * {@code com.foo.bar.MyApp}, and MyApp was a subclass of
	 * {@code SingleFrameApplication.class}, then the ResourceBundle names would
	 * be:</p>
	 * 
	 * <ol>
	 * <li><code>com.foo.bar.resources.MyApp</code></li>
	 * <li><code>javax.swing.application.resources.SingleFrameApplication</code></li>
	 * <li><code>javax.swing.application.resources.Application</code></li>
	 * </ol>
	 * 
	 * <p>The default value of this property is computed lazily and cached. If it's
	 * reset, then all ResourceMaps cached by {@code getResourceMap} will be
	 * updated.</p>
	 * 
	...
	public List<String> getApplicationBundleNames()
	{
		 ...
		 * application's class hasn't been specified yet. In that case we just
	...
	public void setApplicationBundleNames(List<String> bundleNames)
	{
		...
		List<String> oldValue = applicationBundleNames;
		if ( bundleNames != null )
		{
	 ...
	 * sub-package. For example, given a class named com.foo.bar.MyClass, the
	...
	private String classBundleBaseName(Class<?> cls)
	{
		...
		// sb.append("resources.");
		if ( !Application.getBooleanProperty(Application.IGNORE_RESOURCES_PREFIX) )
			sb.append("resources.");
	...
	/**
	 * Map from a class to a list of the names of the {@code ResourceBundles}
	 * specific to the class. The list is in priority order: resources defined
	 * by the first ResourceBundle shadow resources with the the same name that
	 * come later.
	 * <p>
	 * By default this method returns one ResourceBundle whose name is the same
	 * as the class's name, but in the {@code "resources"} subpackage.
	 * <p>
	 ...
	 * given an inner class named {@code com.foo.bar.OuterClass$InnerClass}, the
	 * ResourceBundle name would be {@code "com.foo.bar.resources.InnerClass"}.
	 * <p>
	...
	/**
	 * Called by {@link #getResourceMap} to construct {@code ResourceMaps}. By
	 * default this method is effectively just:
	 * 
	 * <pre>
