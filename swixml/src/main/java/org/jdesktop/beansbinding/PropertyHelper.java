 ...
 * removing, and getting listeners. {@code PropertyHelper} can be constructed to
 * manage listeners for multiple source objects, or to ignore the source object
 * argument when dealing with listeners and associate them directly with the
 * {@code PropertyHelper} instance itself. This makes {@code PropertyHelper}
 * useful as a base for both property types described in the documentation for
 * {@code Property}.
 * 
 * <p>{@code PropertyHelper} also provides, by way of the protected methods
 ...
 *
 * @apiNote {@code <S>} the type of source object that this {@code Property} operates on
 * @apiNote {@code <V>} the type of value that this {@code Property} represents
 */
public abstract class PropertyHelper<S, V> extends Property<S, V>
{
	...
	/**
	 * Create a {@code PropertyHelper}, specifying whether it manages listeners
	 * for multiple source objects, or ignores the source object argument when
	 * dealing with listeners
	 *
	 * @param ignoresSource whether or not the source argument is ignored when
	 *            dealing with listeners
	 */
	...
	/**
	 * Called when this {@code PropertyHelper} changes from having no listeners
	 * installed for the given source object to having listeners installed for
	 * the given source object. This is the ideal time for subclasses to install
	 * any listeners needed to track change on the source object.
	 *
	...
	/**
	 * Called when this {@code PropertyHelper} changes from having listeners
	 * installed for the given source object to having no listeners installed
	 * for the given source object. This is the ideal time for subclasses to
	 * remove any listeners that they've installed to track changes on the
	 * source object.
	 *
	 * @see #listeningStopped
	 */
