 ...
 * create a property representing the {@code firstName} property of an object:
 * 
 * <pre><code>
 *    BeanProperty.create("firstName");
 *</code></pre>
 *
 * <p>Or to create a property representing the {@code firstName} property of an
 * object's {@code mother} property:</p>
 * 
 * <pre><code>
 *    BeanProperty.create("mother.firstName");
 * </code></pre>
 * 
 * <p>An instance of {@code BeanProperty} is immutable and can be used with
 * different source objects. When a {@code PropertyStateListener} is added to a
 * {@code BeanProperty} for a given source object, the {@code BeanProperty}
 * starts listening to all objects along the path (based on that source object)
 * for change notification, and reflects any changes by notifying the listener
 * associated with the property for that source object. So, in the second
 ...
 * different), or {@code Duke's mother's firstName} changes.</p>
 * 
 * <p>It is very important that any bean properties addressed via a
 * {@code BeanProperty} follow the Java Beans specification, including firing
 * property change notification; otherwise, {@code BeanProperty} cannot respond
 * to change. As some beans outside of your control may not follow the Java
 * Beans specification, {@code BeanProperty} always checks the
 ...
 * {@code PropertyStateListeners} installed, the beans along the path (including
 * the final value) are cached, and only updated upon notification of change
 * from a bean. Again, this makes it very important that any bean property that
 * could change along the path fires property change notification.</p>
 * 
 * <p><b>Readability</b> of a {@code BeanProperty} for a
 * given source is defined as follows: A {@code BeanProperty} is readable for
 * a given source if and only if a) each bean in the path, starting with the
 * source, defines a Java Beans getter method for the the property to be read on
 * it AND b) each bean in the path, starting with the source and ending with the
 * bean on which we read the final property, is {@code non-null}. The final
 * value being {@code null} does not affect the readability.</p>
 * 
 * <p>So, in the second example given earlier, the {@code BeanProperty} is readable
 ...
 * Beans getter for {@code firstName}, {@code Duke} is {@code non-null},
 * {@code Duke's mother} is {@code non-null}. The {@code BeanProperty} is
 ...
 * define a Java Beans getter for {@code firstName}, {@code Duke} is
 * {@code null}, {@code Duke's mother} is {@code null}.</p>
 * 
 * <p><b>Writability</b> of a {@code BeanProperty} for
 * a given source is defined as follows: A {@code BeanProperty} is writable
 * for a given source if and only if a) each bean in the path, starting with the
 * source and ending with the bean on which we set the final property, defines a
 * Java Beans getter method for the property to be read on it AND b) the bean on
 * which we set the final property defines a Java Beans setter for the property
 * to be set on it AND c) each bean in the path, starting with the source and
 * ending with the bean on which we set the final property, is {@code non-null}.
 * The final value being {@code null} does not affect the writability.</p>
 * 
 * <p>So, in the second example given earlier, the {@code BeanProperty} is
 * writable for {@code Duke} when all of the following are true: {@code Duke}
 * defines a Java Beans getter for {@code mother}, {@code Duke's mother} defines
 * a Java Beans setter for {@code firstName}, {@code Duke} is {@code non-null},
 * {@code Duke's mother} is {@code non-null}. The {@code BeanProperty} is
 * therefore unreadable when any of the following is true: {@code Duke} does not
 * define a Java Beans getter for {@code mother}, {@code Duke's mother} does not
 ...
 * resolve the path. The throwing of this exception represents an abnormal
 * condition and if listeners are installed for the given source object, leaves
 * the {@code BeanProperty} in an inconsistent state for that source object. A
 * {@code BeanProperty} should not be used again for that same source object
 * after such an exception without first removing all listeners associated with
 * the {@code BeanProperty} for that source object.</p>
 *
 * <pre>
 * TO DO LIST:
 *
 ...
 * 
 * @apiNote {@code <S>} the type of source object that this {@code BeanProperty} operates on
 * @apiNote {@code <V>} the type of value that this {@code BeanProperty} represents
 */
public final class BeanProperty<S, V> extends PropertyHelper<S, V>
{
	 ...
	 * @throws IllegalArgumentException if the path is null, or contains no
	 *             property names
	 */
	 ...
	 * @param path the path
	 * @return an instance of {@code BeanProperty} for the given base property
	 *         and path
	 * @throws IllegalArgumentException if the path is null, or contains no
	 *             property names
	 */
	...
	/**
	 * Returns a string representation of the {@code BeanProperty}. This method
	 * is intended to be used for debugging purposes only, and the content and
	...
	private void write(Object writer, Object object, String string, Object value)
	{
		...
		if ( writer instanceof Map )
		{
			...
			((Map<String, Object>) writer).put(string, value);
			return;
		}
		object = getAdapter(object, string);
