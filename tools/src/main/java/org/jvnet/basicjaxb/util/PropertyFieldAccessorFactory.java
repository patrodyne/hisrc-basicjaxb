...
import static org.jvnet.basicjaxb.util.FieldUtils.isPropertyField;
...
public class PropertyFieldAccessorFactory implements FieldAccessorFactory
{
	...
	private static class PropertyFieldAccessor implements FieldAccessorEx
	{
		...
		public void fromRawValue(JBlock block, String uniqueName, JExpression $var)
		{
			...
			else
			{
				...
				if ( fieldOutline.getPropertyInfo().isCollection() )
					fieldAccessor.fromRawValue(block._if($var.ne(JExpr._null()))._then(), uniqueName, $var);
				else
		...
		public void toRawValue(JBlock block, JVar $var)
		{
			...
			else if ( type.isPrimitive() || fieldOutline.getPropertyInfo().isCollection() )
			{
				...
				if ( type.isPrimitive() )
				{
					...
					{
						defaultExpression = defaultValue.compute(fieldOutline.parent().parent());
					}
					else if ( type.fullName().equals(type.owner().BOOLEAN.fullName()) )
					{
				...
				if ( getter != null )
				{
					if ( isAlwaysSet() )
						...
						block.assign($var, JOp.cond(hasSetValue(), targetObject.invoke(getter), defaultExpression));
				}
				else
				{
