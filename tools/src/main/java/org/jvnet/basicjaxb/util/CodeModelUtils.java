...
{
	// Seal this utility class for static use.
	...
	{
		return (theClass.outer() == null ? theClass.name() :
			getDottedLocalClassName(theClass.outer()) + "." + theClass.name());
	}

	public static String getPackagedClassName(final JClass theClass)
	{
		return (theClass.outer() == null ? theClass.fullName() :
			getPackagedClassName(theClass.outer()) + "$" + theClass.name());
	}
	...
	public static JStatement split(JDefinedClass theClass, JStatement[] statements, String prefix, int start,
		int length, int threshold)
	{
		final JMethod method = theClass.method(JMod.PRIVATE + JMod.STATIC, theClass.owner().VOID, prefix);
		if (length < threshold)
		{
		...
		{
			method.body().add(split(theClass, statements, prefix + "_0", start, length / 2, threshold));
			method.body().add(split(theClass, statements, prefix + "_1", start + length / 2, length - (length / 2), threshold));
		}
	...
	public static JMethod getMethod(final ClassOutline classOutline, final String name)
	{
	...
	public static JDefinedClass _getClass(JCodeModel codeModel, String fullyQualifiedName)
	{
		...
		else
		{
			...
			for (Iterator<JPackage> iterator = codeModel.packages(); iterator.hasNext();)
			{
				final JPackage _package = iterator.next();
				if (packageName.equals(_package.name()))
					return _getClass(_package, fullyQualifiedName.substring(idx + 1));
			}
			return null;
		}
	}
	...
	public static JDefinedClass _getClass(JPackage _package, String name)
	{
		...
		else
		{
			...
			final JDefinedClass parentClass = _package._getClass(parentClassName);
			if (parentClass == null)
				return null;
			else
	...
	public static JDefinedClass _getClass(JDefinedClass _class, String name)
	{
		...
		if (idx < 0)
		{
			for (Iterator<JDefinedClass> nestedClasses = _class.classes(); nestedClasses.hasNext();)
			{
		...
		{
			final JDefinedClass nestedClass = _getClass(_class, name.substring(0, idx));
			if (nestedClass == null)
				return null;
			else
	 ...
	 */
	public static void groupMethods(JDefinedClass ownerClass, JMethod targetMethod, JMethod sourceMethod)
	{
		if ( ownerClass.methods() instanceof List )
		{
			...
			{
				int index = methodList.indexOf(targetMethod);
