...
public class ExtendedJAXBEqualsStrategy extends JAXBEqualsStrategy
{
	...
	protected boolean equalsInternal(ObjectLocator leftLocator, ObjectLocator rightLocator, Object lhs, Object rhs)
	{
		if (lhs instanceof Node && rhs instanceof Node)
			return equalsInternal(leftLocator, rightLocator, (Node) lhs, (Node) rhs);
		else if (lhs instanceof XMLGregorianCalendar && rhs instanceof XMLGregorianCalendar)
			return equalsInternal(leftLocator, rightLocator, (XMLGregorianCalendar) lhs, (XMLGregorianCalendar) rhs);
		else
			return super.equalsInternal(leftLocator, rightLocator, lhs, rhs);
	}

	protected boolean equalsInternal(ObjectLocator leftLocator, ObjectLocator rightLocator, XMLGregorianCalendar left,
		XMLGregorianCalendar right)
	{
		return equals(leftLocator, rightLocator, left.normalize().toGregorianCalendar().getTimeInMillis(),
			right.normalize().toGregorianCalendar().getTimeInMillis());
	}
